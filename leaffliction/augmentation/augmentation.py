...
class ImageAugmentor:
    def __init__(self, source_directory,
                 augmented_output_directory="augmented_directory",
    ...
    def apply_rotation(self, image, save_path=None):
        rotated_image = t.functional.rotate(
            image, 30,
    ...
    def adjust_brightness(self, image, save_path=None):
        ...
        if save_path:
            write_jpeg(brightness_adjusted_image,
                       f"{save_path}_Brightness.jpg")
    ...
    def apply_perspective_transform(self, image, save_path=None):
        ...
        if save_path:
            write_jpeg(perspective_transformed_image,
                       f"{save_path}_Distortion.jpg")
    ...
    def augment_images_in_class(self, max_augmentations, plant_name,
                                disease_class, image_paths):
        """
        Augments images in the specified class directory.
        """
        class_output_path = os.path.join(self.augmented_output_directory,
                                         plant_name, disease_class)
    ...
    def gather_image_paths(self):
        ...
        for plant_name in os.listdir(self.source_directory):
            plant_directory_path = os.path.join(self.source_directory,
                                                plant_name)
            for disease_class in os.listdir(plant_directory_path):
                disease_directory_path = os.path.join(plant_directory_path,
                                                      disease_class)
                for image_file in os.listdir(disease_directory_path):
                    image_file_path = os.path.join(disease_directory_path,
                                                   image_file)
    ...
    def create_image_dataframe(self, image_paths):
        ...
        """
        image_dataframe = pd.DataFrame(image_paths,
                                       columns=['image_file_path'])
        image_dataframe['plant_name'] = \
            image_dataframe['image_file_path'].apply(
                lambda x: x.split(os.sep)[-3])
        image_dataframe['disease_class'] = \
            image_dataframe['image_file_path'].apply(
    ...
    def stratified_sample_dataframe(self, dataframe, column_name, sample_size):
        ...
        min_sample_size = min(sample_size,
                              dataframe[column_name].value_counts().min())
        stratified_sample = dataframe.groupby(column_name).apply(
            lambda x: x.sample(min_sample_size))
        stratified_sample.index = stratified_sample.index.droplevel(0)
        return stratified_sample

    def balance_and_split_dataset(self, no_validation=False):
        """
        Splits data into training & validation sets and applies augmentations.
        If no_validation is set to True, it will only create the training set,
        without splitting into validation.
        """
        image_paths = self.gather_image_paths()
        image_dataframe = self.create_image_dataframe(image_paths)
        image_dataframe_groupby = image_dataframe.groupby('plant_name')

        for plant_name, group_dataframe in image_dataframe_groupby:
            augmentation_count = {}

            if not no_validation:
                validation_dataframe = self.stratified_sample_dataframe(
                    group_dataframe, 'disease_class', 25)
                training_dataframe = group_dataframe.drop(
                    validation_dataframe.index)
            else:
                training_dataframe = group_dataframe

            disease_value = training_dataframe['disease_class'].value_counts()
            max_augmentations_per_class = \
                (disease_value.min()
                    * self.number_of_transformations)

            for disease_class in disease_value.index:
                augmentations_needed = (max_augmentations_per_class
                                        - disease_value.loc[disease_class])
                augmentation_count[disease_class] = augmentations_needed

            for image_path, disease_class in zip(
                    training_dataframe['image_file_path'],
                    training_dataframe['disease_class']):
                training_output_path = os.path.join(
                    self.augmented_output_directory,
                    plant_name, 'train', disease_class)
            ...
            if not no_validation:
                for image_path, disease_class in zip(
                        validation_dataframe['image_file_path'],
                        validation_dataframe['disease_class']):
                    validation_output_path = os.path.join(
                        self.validation_output_directory,
                        plant_name, disease_class)
    ...
    def create_balanced_dataset(self):
        ...
        for plant_name, group_dataframe in image_dataframe_groupby:
            ...
            disease_value_counts = \
                group_dataframe['disease_class'].value_counts()
            min_class_count = disease_value_counts.min()
            max_augmentations_per_class = (min_class_count
                                           * self.number_of_transformations)

            for disease_class in disease_value_counts.index:
                augmentations_needed = \
                    (max_augmentations_per_class
                        - disease_value_counts.loc[disease_class])
                print(f"{disease_class}: {augmentations_needed}\
                       augmentations needed.")
                self.augment_images_in_class(
                    augmentations_needed, plant_name, disease_class,
                    image_dataframe[
                        (image_dataframe['disease_class'] == disease_class)
                        & (image_dataframe['plant_name'] == plant_name)]
                    ['image_file_path']
                )
    ...
    def augment_single_image(self, image_path):
        """
        Applies augmentations to a single image
        and displays the original and augmented images.
        """
        image = read_image(image_path)
        augmented_images = [transformation(image) for transformation
                            in self.transformation_functions]
        labels = ['Flip', 'Rotate', 'Blur', 'Contrast',
                  'Brightness', 'Perspective', 'Scaling']

        # Show original image along with augmented images
        self.display_images_with_labels([image] + augmented_images,
                                        labels=['Original'] + labels)

        # Save the augmented images
        for i, augmented_image in enumerate(augmented_images):
            write_jpeg(augmented_image,
                       f"{os.path.splitext(image_path)[0]}_{labels[i]}.jpg")


def parse_arguments():
    parser = argparse.ArgumentParser(
        prog='ImageAugmentation',
        description='Program that applies augmentations to a \
        single image or creates a balanced dataset from a directory of images.'
    )
    parser.add_argument('source_directory', metavar='source_directory',
                        type=str, nargs=1,
                        help='Directory containing images to augment.')
    parser.add_argument('-no_validation', action='store_true',
                        help='If set, the dataset will not be \
