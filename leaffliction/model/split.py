...
def create_dir_structure(root_dir, categories):
    """
    Create the folder structure for each category
...
def split_dataset(dataset_dir, output_dir, split_ratio):
    ...
    for category in categories:
        ...
        if 'Apple' in category:
            train_category_path = os.path.join(apples_dir,
                                               'training', category)
            val_category_path = os.path.join(apples_dir,
                                             'validation', category)
        elif 'Grape' in category:
            train_category_path = os.path.join(grapes_dir,
                                               'training', category)
            val_category_path = os.path.join(grapes_dir,
                                             'validation', category)
        else:
            continue
...
def copy_folders(dataset_dir, output_dirs):
    ...
    """
    categories = {
        "Grapes": ["Grape_Black_rot", "Grape_Esca",
                   "Grape_healthy", "Grape_spot"],
        "Apples": ["Apple_Black_rot", "Apple_healthy",
...
if __name__ == "__main__":
    try:
        parser = argparse.ArgumentParser(
            description="Split dataset or copy folders.")
        parser.add_argument("dataset_dir",
                            help="Path to the dataset directory\
                                  containing category subdirectories.")
        parser.add_argument("output_dir",
                            help="Base path for the output directory. \
                                The script will create 'dataset' \
                                    or 'datasets' inside this base path.")
        parser.add_argument("--split_ratio", type=float, default=0.85,
                            help="Percentage of data to be used for training.\
                                  Default is 80%.")
        parser.add_argument("-data", action="store_true",
                            help="If used, split the dataset into training \
                                and validation sets. Otherwise, copy \
                                folders like the bash script.")

        args = parser.parse_args()

        # Set the appropriate directory structure
        # (either 'dataset' or 'datasets')
        if args.data:
        ...
        if args.data:
            split_dataset(args.dataset_dir, wrapped_output_dir,
                          args.split_ratio)
        else:
