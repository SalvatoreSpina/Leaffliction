...
class ImageTransformer:
    def __init__(self, source_directory, destination_directory):
        ...
        with source and destination directories."""
    ...
    def apply_gaussian_blur(self, image):
        ...
        binary_threshold = pcv.threshold.binary(gray_img=saturation_channel,
                                                threshold=40, max_value=255,
                                                object_type='light')
        gaussian_blurred_image = pcv.gaussian_blur(img=binary_threshold,
                                                   ksize=(5, 5), sigma_x=0,
                                                   sigma_y=None)
    ...
    def create_image_mask(self, img):
        ...
        binary_threshold = pcv.threshold.binary(gray_img=saturation_channel,
                                                threshold=40, max_value=255,
                                                object_type='dark')
        median_blurred_image = pcv.median_blur(gray_img=binary_threshold,
                                               ksize=5)
        blue_channel = pcv.rgb2gray_lab(rgb_img=img, channel='b')

        blue_threshold = pcv.threshold.binary(gray_img=blue_channel,
                                              threshold=120, max_value=255,
                                              object_type='dark')
        combined_binary = pcv.logical_or(bin_img1=median_blurred_image,
                                         bin_img2=blue_threshold)
        ...
        masked_a_threshold = pcv.threshold.binary(gray_img=masked_a_channel,
                                                  threshold=115, max_value=255,
                                                  object_type='dark')
        masked_a_threshold1 = pcv.threshold.binary(gray_img=masked_a_channel,
                                                   threshold=135,
                                                   max_value=255,
                                                   object_type='light')
        masked_b_threshold = pcv.threshold.binary(gray_img=masked_b_channel,
                                                  threshold=128, max_value=255,
                                                  object_type='light')
        combined_ab1 = pcv.logical_or(bin_img1=masked_a_threshold,
                                      bin_img2=masked_b_threshold)
        combined_ab = pcv.logical_or(bin_img1=masked_a_threshold1,
                                     bin_img2=combined_ab1)
        filled_mask = pcv.fill(bin_img=combined_ab, size=200)
        final_masked_image = pcv.apply_mask(img=masked_image, mask=filled_mask,
                                            mask_color='white')
    ...
    def save_transformed_image(self, image, transformation_name, filename):
        ...
        output_path = os.path.join(self.destination_directory,
                                   f"{filename}_{transformation_name}.png")
        print(output_path)
    ...
    def process_single_image(self, image_path, args):
        ...
        for key, transformation in TRANSFORMATIONS.items():
            if args[key] or args['apply_all']:
                ...
                # If the transformation returns a tuple,
                # unpack it to get the actual image
    ...
    def process_directory_of_images(self, args):
        """Process all image files in a
        directory by applying transformations."""
...
def parse_command_line_arguments():
    ...
    parser = argparse.ArgumentParser(
        ...
        description='Program to apply various image transformations \
            to a file or directory of files.'
    )
...
if __name__ == '__main__':
    try:
        ...
        # with source and destination directories
        transformer = ImageTransformer(args['source_directory'],
                                       args['destination_directory'])

        if not os.path.exists(args['source_directory']):
            sys.exit('The specified source directory \
        ...
        if os.path.isfile(args['source_directory']):
            ...
            # This line should show the image in the GUI but it has some issues
            # Probably due to wrong environment.
            # Uncomment the print and use the command manually
            # script = f"python3 tester.py --show
            # {shlex.quote(args['source_directory'])}
            # {shlex.quote(args['destination_directory'])}"
            # print(script)
            # os.system(script)
